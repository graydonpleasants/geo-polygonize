...
fn test_nan_deduplication_failure() {
    ...
    let l_nan = Line::new(Coord { x: f64::NAN, y: 0.0 }, Coord { x: 10.0, y: 10.0 });
    let l_inf = Line::new(Coord { x: f64::INFINITY, y: 0.0 }, Coord { x: 10.0, y: 10.0 });
    ...
    assert_eq!(result.len(), 1, "Expected 1 line after filtering NaNs and Infs, found {}", result.len());
