...
## 2026-03-24 - [Avoid Manual Power-of-Two Reciprocal Multiplication]
**Learning:** Manually changing floating point division by a power of two (like `/ 2.0`) to multiplication by its reciprocal (`* 0.5`) provides absolutely no real-world performance benefit because LLVM automatically performs this optimization under all math settings without loss of precision. However, LLVM *cannot* automatically extract an arbitrary reciprocal (e.g. `let inv = 1.0 / (3.0 * x)`) to replace multiple identical divisions without `ffast-math` enabled due to floating-point non-associativity.
**Action:** When optimizing Rust floating-point math, ignore power-of-two divisions as they are already optimized by the compiler. Instead, focus on extracting and caching arbitrary reciprocals to manually replace multiple identical divisions with cheaper multiplications.
