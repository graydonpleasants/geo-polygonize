...
## 2024-05-30 - Pre-allocating Capacity for Nested Composite Iterators
**Learning:** In code paths heavily allocating collections for recursive or compound geometric entities (like mapping over a `MultiPolygon`'s sub-polygons to extract their `LineString` elements), iterating over them to push values dynamically forces the vector to repeatedly reallocate. However, because `.len()` on these inner representations (like `LineString(Vec<Coord>)`) is O(1), summing these lengths upfront is extremely fast and completely avoids O(N) reallocation costs.
**Action:** Always sum the exact length required by iterating the nested structures (e.g., using `iter().map(|item| item.len()).sum()`), and make a single `.reserve()` call on the target `Vec` before beginning iteration to prevent the overhead of continuous array resizing in hot loops.
