...
## 2024-05-30 - In-place rotation for ring canonicalization
**Learning:** Using `extend_from_slice` and `Vec::with_capacity` to cyclically shift coordinate rings allocates new memory and performs unneeded memory copies. This is particularly inefficient for functions called repeatedly like `canonicalize_ring`.
**Action:** Use `slice::rotate_left(min_idx)` (or `rotate_right`) to perform a zero-allocation, in-place shift. Ensure you maintain structural integrity bounds depending on whether the ring natively repeats its closing vertex (`ring[n] = ring[0]`) or requires a manual push (`ring.push(ring[0])`).
