...
**Action:** To safely eliminate array bounds checks in Rust while preserving readability during a sequence scan (like finding the minimum index of a collection using `min_by`), prefer iterating over the slice directly (e.g., `slice.iter().enumerate().min_by(...)`) instead of using an index range `(0..n)` which forces the compiler to insert bounds checks. Never sacrifice readability by converting these to manual `for` loops just for micro-optimizations.## 2024-04-02 - ContainmentForest Bounding Box Redundancy
**Learning:** `ContainmentForest::new` and topology resolution phases independently calculated `bounding_rect_3d` for the same exterior rings, causing an unnecessary O(N) traversal per shell per phase.
**Action:** When designing algorithms with multiphase spatial processing, cache AABBs alongside spatial indexes instead of only putting them inside the index structure, as querying the index may sometimes require retrieving the original geometry's bounds again.
