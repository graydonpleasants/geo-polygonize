...
## 2024-05-18 - [Floating Point Reciprocal Optimization]
**Learning:** In Rust (and LLVM generally), floating point division (`/`) cannot be automatically optimized into multiplication by a reciprocal (`* .recip()`) within strict logic paths due to IEEE-754 associativity rules, unless `ffast-math` is enabled. In hot geometric loops (like computing area and centroids), this causes a measurable CPU overhead because division is significantly more expensive than multiplication.
**Action:** When performing repeated divisions by the same constant or calculated value (e.g., dividing coordinates by total area), explicitly calculate the reciprocal once (`let inv = value.recip();`) and use multiplication to improve performance.
