...
pub fn from_polygonizer_error(e: PolygonizeError) -> JsValue {
    let name = match &e {
        ...
        PolygonizeError::UnsupportedOptionCombination { .. } => "UnsupportedOptionCombination".to_string(),
        PolygonizeError::TopologyFailure { .. } => "TopologyFailure".to_string(),
        PolygonizeError::InternalInvariantViolation { .. } => "InternalInvariantViolation".to_string(),
