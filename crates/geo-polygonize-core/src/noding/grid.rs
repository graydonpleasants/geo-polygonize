                    ...
                    if let Some(res) = line_intersection(l1_2d, l2_2d) {
                        match res {
                            ...
                            LineIntersection::Collinear {
                                ...
                                // Note: SnapNoder::snap is 2D, returns Coord<f64>.
                                // But handle_intersection expects Coord3D in callback?
                                // Actually handle_intersection will now handle 3D interpolation.
                                // But overlap.start/end are 2D points from intersection.
                                // We need to handle this carefully in SnapNoder.
                                // For grid usage, we just pass the 2D intersection result.

                                let p1 = snap_noder.snap(overlap.start.into());
