...
pub fn polygonize_geoparquet_file(
    ...
    let file = File::open(input_path).map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;

    let builder = ParquetRecordBatchReaderBuilder::try_new(file)
        .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;

    let geo_meta = builder.geoparquet_metadata()
        .transpose()
        .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
    ...
    let _schema = if let Some(meta) = &geo_meta {
        ...
        builder.geoarrow_schema(meta, true, CoordType::Interleaved).map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?
    } else {
        builder.schema().clone()
    };

    let reader = builder.build().map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;

    let out_file = File::create(output_path).map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
    let mut parquet_writer: Option<ArrowWriter<File>> = None;
    let mut gpq_encoder: Option<GeoParquetRecordBatchEncoder> = None;

    for batch_res in reader {
        let batch: RecordBatch = batch_res.map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
        ...
        if parquet_writer.is_none() {
            ...
            let encoder = GeoParquetRecordBatchEncoder::try_new(&new_schema, &writer_options)
                .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
            let writer = ArrowWriter::try_new(out_file.try_clone().unwrap(), encoder.target_schema().clone(), None)
                .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
            parquet_writer = Some(writer);
            gpq_encoder = Some(encoder);
        }

        let encoded_batch = gpq_encoder.as_mut().unwrap().encode_record_batch(&new_batch)
            .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;

        parquet_writer.as_mut().unwrap().write(&encoded_batch)
            .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
    }

    if let (Some(mut writer), Some(encoder)) = (parquet_writer, gpq_encoder) {
        let kv_metadata = encoder.into_keyvalue()
            .map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
        writer.append_key_value_metadata(kv_metadata);
        writer.close().map_err(|e| PolygonizeError::InvalidGeometry { reason: e.to_string() })?;
    }
