...
/// One directed physical edge, including the payload needed to validate adoption.
#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct PartitionBorderPhysicalTopologyEdge {
    pub(crate) from: [u64; 2],
    pub(crate) to: [u64; 2],
    pub(crate) from_z_bits: u64,
    pub(crate) to_z_bits: u64,
    pub(crate) source_line_ids: Vec<u32>,
    pub(crate) symmetric: usize,
...
pub(crate) struct PartitionBorderGlobalArrangementWitness {
    ...
    pub(crate) physical_edges: Vec<PartitionBorderPhysicalTopologyEdge>,
    pub(crate) physical_edge_by_local_edge: Vec<usize>,
}
...
/// One private global edge-topology record materialized from validated
/// successor and face-ID buffers. This does not rewrite local topology or
/// public output.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceTopologyEdge {
    pub(crate) global_dir_edge_id: usize,
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceTopologyStats {
    pub(crate) edge_count: usize,
    pub(crate) next_link_count: usize,
    pub(crate) face_id_count: usize,
    pub(crate) missing_next_count: usize,
    pub(crate) invalid_next_count: usize,
    pub(crate) duplicate_next_count: usize,
    pub(crate) node_discontinuity_count: usize,
...
/// One private shell/hole classification for a detached canonical ring.
/// Unbounded rings are retained as neither output candidate.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceRingClassification {
    pub(crate) candidate_global_face_id: usize,
    pub(crate) cycle_start_global_dir_edge_id: usize,
    pub(crate) containment_depth: usize,
    pub(crate) shell_candidate: bool,
    pub(crate) hole_candidate: bool,
    pub(crate) unbounded: bool,
}

/// Counts the fail-closed classification of detached canonical ring payloads.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceRingClassificationStats {
    pub(crate) cycle_count: usize,
    pub(crate) classified_cycle_count: usize,
    pub(crate) shell_candidate_count: usize,
    pub(crate) hole_candidate_count: usize,
    pub(crate) unbounded_cycle_count: usize,
    ...
    pub(crate) ambiguous_interaction_count: usize,
    pub(crate) missing_interior_point_count: usize,
    pub(crate) invalid_cycle_count: usize,
    pub(crate) evidence_mismatch_count: usize,
    pub(crate) classification_ready: bool,
}

/// One private shell with its deterministically assigned hole candidates.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceRingCandidateAssembly {
...
/// Counts atomic shell/hole candidate assembly after ring classification.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub(crate) struct PartitionBorderGlobalFaceRingCandidateAssemblyStats {
    pub(crate) cycle_count: usize,
    pub(crate) shell_candidate_count: usize,
    pub(crate) hole_candidate_count: usize,
    pub(crate) assembled_shell_count: usize,
    pub(crate) assigned_hole_count: usize,
    pub(crate) unassigned_hole_count: usize,
    pub(crate) ambiguous_hole_count: usize,
    pub(crate) evidence_mismatch_count: usize,
...
pub struct PartitionBorderGraph {
    ...
    global_face_topology_edges: Vec<PartitionBorderGlobalFaceTopologyEdge>,
...
impl PartitionBorderGraph {
    pub fn insert(&mut self, half_edge: PartitionBorderHalfEdge) -> crate::Result<()> {
        ...
        self.global_face_topology_edges.clear();
    ...
    pub(crate) fn global_arrangement_witness(
        ...
        let mut physical_edges = Vec::with_capacity(graph.directed_edges.len());
        let mut slot_by_direction = BTreeMap::new();
        ...
        for (index, edge) in graph.directed_edges.iter().enumerate() {
            ...
            let Some(next) = graph.directed_edges[edge.sym_idx].next_idx else {
                return Ok(None);
            };
            slot_by_direction.insert((key(edge.src), key(edge.dst)), index);
            physical_edges.push(PartitionBorderPhysicalTopologyEdge {
                from: key(edge.src).xy_bits,
                to: key(edge.dst).xy_bits,
                from_z_bits: graph.nodes_z[edge.src].to_bits(),
                to_z_bits: graph.nodes_z[edge.dst].to_bits(),
                source_line_ids: graph.edges[edge.edge_idx].sources.line_ids.to_vec(),
                symmetric: edge.sym_idx,
                next,
                face_id,
            });
        }
        let mut physical_edge_by_local_edge = Vec::with_capacity(self.global_face_edge_map.len());
        ...
        for (index, edge) in self.global_face_edge_map.iter().enumerate() {
            ...
            physical_edge_by_local_edge.push(slot_by_direction[&(edge.from, edge.to)]);
        }
    ...
    pub(crate) fn adopt_global_arrangement_candidate(
        &mut self,
        witness: &PartitionBorderGlobalArrangementWitness,
        claims_ready: bool,
        execution_policy: &ExecutionPolicy,
    ) -> crate::Result<Option<PartitionBorderGlobalTopologyCandidateStats>> {
        let stage = "partition_border_global_arrangement_adoption";
        execution_policy.check_cancelled(stage)?;
        let edge_count = self.global_face_edge_map.len();
        execution_policy.check(stage, execution_policy.max_graph_edges, edge_count)?;
        execution_policy.check(stage, execution_policy.max_graph_nodes, witness.face_count)?;
        if !claims_ready
            || edge_count == 0
            || witness.physical_edges.len() != edge_count
            || witness.physical_edge_count.checked_mul(2) != Some(edge_count)
            || witness.node_count != self.global_face_nodes.len()
            || witness.node_count as i128 - witness.physical_edge_count as i128
                + witness.face_count as i128
                != 2
            || witness.physical_edge_by_local_edge.len() != edge_count
            || witness.face_ids_by_local_edge.len() != edge_count
            || witness.unbounded_face_ids.len() != 1
            || self.global_topology_candidate.is_none()
            || !self.global_next_global_dir_edge_ids.is_empty()
        {
            return Ok(None);
        }
        ...
        for start in 0..edge_count {
            execution_policy.check_cancelled_every(stage, start)?;
            if visited[start] {
                continue;
            }
            let face = witness.face_ids_by_local_edge[start];
            ...
            loop {
                execution_policy.check_cancelled_every(stage, coords.len())?;
                if visited[current] {
                    if current != start {
                        return Ok(None);
                    }
        ...
        if faces.len() != witness.face_count || exterior != witness.unbounded_face_ids {
            return Ok(None);
        }
        let incomplete_application_plan_count = self
            .global_face_next_application_plans
            .iter()
            .filter(|plan| !plan.closed || !plan.node_continuous)
            .count();
        let stats = PartitionBorderGlobalTopologyCandidateStats {
            edge_count,
            local_successor_count: self
                .global_face_edge_map
                .iter()
                .filter(|edge| edge.local_face_successor_global_dir_edge_id.is_some())
                .count(),
            global_override_count: self
                .global_face_edge_map
                .iter()
                .zip(&next)
                .filter(|(edge, successor)| {
                    edge.local_face_successor_global_dir_edge_id != **successor
                })
                .count(),
            assigned_next_count: edge_count,
            cycle_count: starts.len(),
            closed_cycle_edge_count: edge_count,
            incomplete_application_plan_count,
            candidate_ready: incomplete_application_plan_count == 0,
            ..Default::default()
        };
        execution_policy.check_cancelled(stage)?;
        self.global_topology_candidate = Some(PartitionBorderGlobalTopologyCandidate {
            next_global_dir_edge_ids: next,
            cycle_start_global_dir_edge_ids: starts,
        });
        Ok(Some(stats))
    }
    ...
    /// Classifies retained canonical rings by winding and containment. This is
    /// private evidence only: no topology or public tiled output is changed.
    pub(crate) fn classify_global_face_ring_payloads(
        &mut self,
        execution_policy: &ExecutionPolicy,
        payload_stats: PartitionBorderGlobalFaceRingPayloadStats,
        geometry_stats: PartitionBorderGlobalFaceCycleGeometryStats,
    ) -> crate::Result<PartitionBorderGlobalFaceRingClassificationStats> {
        execution_policy.check_cancelled("partition_border_global_face_ring_classification")?;
        let cycle_count = payload_stats.cycle_count;
        execution_policy.check(
            "partition_border_global_face_ring_classification_cycles",
            execution_policy.max_graph_nodes,
            cycle_count,
        )?;
        let mut stats = PartitionBorderGlobalFaceRingClassificationStats {
            cycle_count,
            ..Default::default()
        };
        ...
        let pair_work = cycle_count.checked_mul(cycle_count).ok_or_else(|| {
            crate::PolygonizeError::InternalInvariantViolation {
                reason: "global face ring classification pair count overflow".to_string(),
            }
        })?;
        execution_policy.check(
            "partition_border_global_face_ring_classification_pairs",
            execution_policy.max_candidate_pairs,
            pair_work,
        )?;

        let mut rings = Vec::with_capacity(cycle_count);
        for (index, payload) in self.global_face_ring_payloads.iter().enumerate() {
            execution_policy.check_cancelled_every(
                "partition_border_global_face_ring_classification_cycles",
                index,
            )?;
            let coords = payload
                .coords
                .iter()
                .map(|bits| Coord3D::new(f64::from_bits(bits[0]), f64::from_bits(bits[1]), 0.0))
                .collect::<Vec<_>>();
            let area = Polygon3D::ring_signed_area_2d(&coords);
            if coords.len() < 4
            ...
            let polygon = Polygon::new(
                LineString(coords.iter().map(|coord| coord.to_coord_2d()).collect()),
                Vec::new(),
            );
            if polygon.interior_point().is_none() {
                stats.missing_interior_point_count += 1;
            ...
            rings.push(Some((polygon, area)));
        }

        let mut contained_cycles = BTreeSet::new();
        let mut containment_depths = vec![0usize; cycle_count];
        for (outer_index, outer_ring) in rings.iter().enumerate() {
            let Some((outer, outer_area)) = outer_ring else {
                continue;
            };
            for (inner_index, inner_ring) in rings.iter().enumerate() {
                execution_policy.check_cancelled_every(
                    "partition_border_global_face_ring_classification_pairs",
                    outer_index * cycle_count + inner_index,
                )?;
                let Some((inner, inner_area)) = inner_ring else {
                    continue;
                };
                if outer_index == inner_index || !outer.contains(inner.exterior()) {
                    continue;
                ...
                stats.containment_pair_count += 1;
                contained_cycles.insert(inner_index);
                containment_depths[inner_index] += 1;
                if outer_area.is_sign_positive() == inner_area.is_sign_positive() {
                    stats.nested_same_orientation_pair_count += 1;
                }
            }
        ...
        stats.contained_cycle_count = contained_cycles.len();
        stats.ambiguous_interaction_count = geometry_stats
            .proper_crossing_count
            .saturating_add(geometry_stats.endpoint_touch_count)
            .saturating_add(geometry_stats.boundary_touch_count)
            .saturating_add(geometry_stats.unexpected_collinear_overlap_count)
            .saturating_add(geometry_stats.self_intersection_count);
        stats.evidence_mismatch_count += usize::from(
            stats.containment_pair_count != geometry_stats.containment_pair_count
                || stats.contained_cycle_count != geometry_stats.contained_cycle_count
                || stats.nested_same_orientation_pair_count
                    != geometry_stats.nested_same_orientation_pair_count,
        );

        let mut classifications = Vec::with_capacity(cycle_count);
        for (index, payload) in self.global_face_ring_payloads.iter().enumerate() {
            let Some(Some((_, area))) = rings.get(index) else {
                continue;
            };
            let shell_candidate = !payload.unbounded && *area > 0.0;
            let hole_candidate = !payload.unbounded && *area < 0.0;
            stats.shell_candidate_count += usize::from(shell_candidate);
            stats.hole_candidate_count += usize::from(hole_candidate);
            stats.unbounded_cycle_count += usize::from(payload.unbounded);
            classifications.push(PartitionBorderGlobalFaceRingClassification {
                candidate_global_face_id: payload.candidate_global_face_id,
                cycle_start_global_dir_edge_id: payload.cycle_start_global_dir_edge_id,
                containment_depth: containment_depths[index],
                shell_candidate,
                hole_candidate,
                unbounded: payload.unbounded,
            });
        }
        stats.classified_cycle_count = classifications.len();
        stats.classification_ready = payload_stats.materialization_ready
            && geometry_stats.ring_payload_ready
            && stats.classified_cycle_count == cycle_count
            && stats.unbounded_cycle_count == 1
            && stats.invalid_cycle_count == 0
            && stats.missing_interior_point_count == 0
            && stats.ambiguous_interaction_count == 0
            && stats.nested_same_orientation_pair_count == 0
            && stats.evidence_mismatch_count == 0;
        if stats.classification_ready {
            self.global_face_ring_classifications = classifications;
        }
        Ok(stats)
    }

    /// Assembles private shell-to-hole candidate groups from classified rings.
    /// The groups are evidence only and are committed atomically.
    pub(crate) fn assemble_global_face_ring_candidates(
        &mut self,
        execution_policy: &ExecutionPolicy,
        classification_stats: PartitionBorderGlobalFaceRingClassificationStats,
    ) -> crate::Result<PartitionBorderGlobalFaceRingCandidateAssemblyStats> {
        execution_policy.check_cancelled("partition_border_global_face_ring_candidate_assembly")?;
        let cycle_count = classification_stats.cycle_count;
        execution_policy.check(
            "partition_border_global_face_ring_candidate_assembly_cycles",
            execution_policy.max_graph_nodes,
            cycle_count,
        )?;
        let mut stats = PartitionBorderGlobalFaceRingCandidateAssemblyStats {
            cycle_count,
            shell_candidate_count: classification_stats.shell_candidate_count,
            hole_candidate_count: classification_stats.hole_candidate_count,
            ..Default::default()
        };
        if !classification_stats.classification_ready
            || self.global_face_ring_payloads.len() != cycle_count
        ...
        let pair_work = cycle_count.checked_mul(cycle_count).ok_or_else(|| {
            crate::PolygonizeError::InternalInvariantViolation {
                reason: "global face ring candidate pair count overflow".to_string(),
            }
        })?;
        execution_policy.check(
            "partition_border_global_face_ring_candidate_assembly_pairs",
            execution_policy.max_candidate_pairs,
            pair_work,
        )?;

        let mut polygons = Vec::with_capacity(cycle_count);
        for (index, payload) in self.global_face_ring_payloads.iter().enumerate() {
            execution_policy.check_cancelled_every(
                "partition_border_global_face_ring_candidate_assembly_cycles",
                index,
        ...
        let shell_indices = self
            .global_face_ring_classifications
            .iter()
            .enumerate()
            .filter_map(|(index, classification)| classification.shell_candidate.then_some(index))
            .collect::<Vec<_>>();
        let hole_indices = self
            .global_face_ring_classifications
            .iter()
            .enumerate()
            .filter_map(|(index, classification)| classification.hole_candidate.then_some(index))
            .collect::<Vec<_>>();
        let mut holes_by_shell = vec![Vec::new(); shell_indices.len()];
        for (hole_work, &hole_index) in hole_indices.iter().enumerate() {
            execution_policy.check_cancelled_every(
                "partition_border_global_face_ring_candidate_assembly_pairs",
                hole_work,
            )?;
            let containing_shells = shell_indices
                .iter()
                .enumerate()
                .filter_map(|(shell_index, &candidate_index)| {
                    polygons[candidate_index]
                        .contains(polygons[hole_index].exterior())
                        .then_some(shell_index)
                })
                .collect::<Vec<_>>();
            match containing_shells.as_slice() {
                [] => stats.unassigned_hole_count += 1,
                [shell_index] => {
                    holes_by_shell[*shell_index].push(hole_index);
                    stats.assigned_hole_count += 1;
                }
                _ => stats.ambiguous_hole_count += 1,
            }
        }

        let mut assemblies = Vec::with_capacity(shell_indices.len());
        for (shell_index, &payload_index) in shell_indices.iter().enumerate() {
            let classification = &self.global_face_ring_classifications[payload_index];
            let mut hole_candidate_global_face_ids = holes_by_shell[shell_index]
                .iter()
                .map(|&hole_index| {
                    self.global_face_ring_classifications[hole_index].candidate_global_face_id
                })
                .collect::<Vec<_>>();
            hole_candidate_global_face_ids.sort_unstable();
            assemblies.push(PartitionBorderGlobalFaceRingCandidateAssembly {
                shell_candidate_global_face_id: classification.candidate_global_face_id,
                shell_cycle_start_global_dir_edge_id: classification.cycle_start_global_dir_edge_id,
                hole_candidate_global_face_ids,
            });
        }
        stats.assembled_shell_count = assemblies.len();
        stats.candidate_ready = stats.evidence_mismatch_count == 0
            && stats.unassigned_hole_count == 0
            && stats.ambiguous_hole_count == 0
            && stats.assembled_shell_count == stats.shell_candidate_count
            && stats.assigned_hole_count == stats.hole_candidate_count;
        if stats.candidate_ready {
            self.global_face_ring_candidate_assemblies = assemblies;
        }
        Ok(stats)
    }
    ...
    #[cfg(test)]
    pub(crate) fn global_face_ring_classifications(
        &self,
    ) -> &[PartitionBorderGlobalFaceRingClassification] {
        &self.global_face_ring_classifications
    }

    #[cfg(test)]
    pub(crate) fn global_face_ring_candidate_assemblies(
        &self,
    ...
    /// Materializes one private global edge-topology record per validated
    /// detached edge. The commit combines the already-proven successor and
    /// per-edge face-ID buffers without rewriting local topology or output.
    pub(crate) fn materialize_global_face_topology(
        &mut self,
        execution_policy: &ExecutionPolicy,
        extraction_gate: PartitionBorderGlobalFaceExtractionGateStats,
    ) -> crate::Result<PartitionBorderGlobalFaceTopologyStats> {
        execution_policy.check_cancelled("partition_border_global_face_topology")?;
        let edge_count = self.global_face_edge_map.len();
        execution_policy.check(
            "partition_border_global_face_topology_edges",
            execution_policy.max_graph_edges,
            edge_count,
        )?;
        let mut stats = PartitionBorderGlobalFaceTopologyStats {
            edge_count,
            ..Default::default()
        };
        if !extraction_gate.extraction_ready || extraction_gate.edge_count != edge_count {
            stats.evidence_mismatch_count += 1;
            return Ok(stats);
        }
        ...
        if self.global_face_id_by_global_dir_edge_id.len() != edge_count {
            stats.evidence_mismatch_count += 1;
        }
        ...
        for edge_index in 0..edge_count {
            execution_policy
                .check_cancelled_every("partition_border_global_face_topology_edges", edge_index)?;
            let Some(next_global_dir_edge_id) = self
                .global_next_global_dir_edge_ids
                .get(edge_index)
                .copied()
                .flatten()
            else {
                stats.missing_next_count += 1;
                continue;
            };
            if next_global_dir_edge_id >= edge_count {
                stats.invalid_next_count += 1;
                continue;
            }
            stats.next_link_count += 1;
            predecessor_counts[next_global_dir_edge_id] += 1;
            if predecessor_counts[next_global_dir_edge_id] > 1 {
                stats.duplicate_next_count += 1;
            }
            match (
                self.global_face_edge_map[edge_index].to_global_node_id,
                self.global_face_edge_map[next_global_dir_edge_id].from_global_node_id,
            ) {
                (Some(to_global_node_id), Some(from_global_node_id))
                    if to_global_node_id == from_global_node_id => {}
                _ => stats.node_discontinuity_count += 1,
            }
            let Some(global_face_id) = self
                .global_face_id_by_global_dir_edge_id
                .get(edge_index)
                .copied()
                .flatten()
        ...
        stats.non_contiguous_face_id_count =
            expected_face_ids.symmetric_difference(&face_ids).count();
        stats.topology_ready = edge_count > 0
            && extraction_gate.extraction_ready
            && extraction_gate.edge_count == edge_count
            && stats.next_link_count == edge_count
            && stats.face_id_count == edge_count
            && stats.missing_next_count == 0
            && stats.invalid_next_count == 0
            && stats.duplicate_next_count == 0
            && stats.node_discontinuity_count == 0
            && stats.missing_face_id_count == 0
    ...
    #[cfg(test)]
    pub(crate) fn global_face_topology_edges(&self) -> &[PartitionBorderGlobalFaceTopologyEdge] {
        &self.global_face_topology_edges
    }
...
mod tests {
    ...
    #[test]
    fn physical_candidate_adoption_validates_payloads_links_and_rolls_back_rejection() {
        ...
        assert!(graph
            .adopt_global_arrangement_candidate(&witness, false, &policy)
            .unwrap()
            .is_none());
        ...
        for mutation in 0..5 {
            ...
            match mutation {
                0 => invalid.physical_edges[0].next = usize::MAX,
                1 => invalid.physical_edges[0].source_line_ids.clear(),
                2 => invalid.physical_edges[0].from_z_bits = 99.0f64.to_bits(),
                3 => invalid.unbounded_face_ids.clear(),
                _ => {
                    invalid.physical_edge_by_local_edge[0] = invalid.physical_edge_by_local_edge[1]
                }
            }
            assert!(graph
                .adopt_global_arrangement_candidate(&invalid, true, &policy)
                .unwrap()
                .is_none());
            assert_eq!(graph, before);
        }
        ...
        token.cancel();
        assert!(matches!(
            graph.adopt_global_arrangement_candidate(
                &witness,
                true,
                &ExecutionPolicy {
        ...
        assert_eq!(stats.closed_cycle_edge_count, 8);
        assert_ne!(
            graph.global_topology_candidate,
            before.global_topology_candidate
        );
        let mut expected = before;
        expected.global_topology_candidate = graph.global_topology_candidate.clone();
    ...
    #[test]
    fn global_face_topology_materialization_is_atomic_and_output_neutral() {
        let mut graph = exact_global_topology_candidate_graph();
        graph
            .reconcile_global_topology_candidate(&ExecutionPolicy::default())
            .unwrap();
        ...
        let stats = graph
            ...
            .unwrap();
        assert_eq!(stats.edge_count, 4);
        assert_eq!(stats.next_link_count, 4);
        assert_eq!(stats.face_id_count, 4);
        assert!(stats.topology_ready);
        assert_eq!(graph.global_face_topology_edges().len(), 4);
        assert!(graph
        ...
        let before = graph.global_face_topology_edges().to_vec();
        graph.global_face_id_by_global_dir_edge_id = vec![Some(0), Some(0), Some(2), Some(2)];
        let stats = graph
            .materialize_global_face_topology(
                &ExecutionPolicy::default(),
                PartitionBorderGlobalFaceExtractionGateStats {
            ...
            .unwrap();
        assert!(stats.non_contiguous_face_id_count > 0);
        assert!(!stats.topology_ready);
        ...
        graph.global_face_edge_map[0].to_global_node_id = None;
        let stats = graph
            .materialize_global_face_topology(
                &ExecutionPolicy::default(),
                PartitionBorderGlobalFaceExtractionGateStats {
    ...
    #[test]
    fn global_face_topology_materialization_checks_limits_and_cancellation() {
        let mut limited = exact_global_topology_candidate_graph();
        limited
            .reconcile_global_topology_candidate(&ExecutionPolicy::default())
            .unwrap();
        limited.global_next_global_dir_edge_ids = vec![Some(1), Some(0), Some(3), Some(2)];
        limited.global_face_id_by_global_dir_edge_id = vec![Some(0), Some(0), Some(1), Some(1)];
        let gate = PartitionBorderGlobalFaceExtractionGateStats {
            edge_count: 4,
            extraction_ready: true,
            ..Default::default()
        };
        ...
        assert!(matches!(
            ...
            crate::PolygonizeError::ResourceLimitExceeded {
                ...
                limit: 0,
                observed: 4,
            } if stage == "partition_border_global_face_topology_edges"
        ));
    ...
    #[test]
    fn global_face_ring_payloads_classify_nested_shell_and_unbounded_evidence() {
        let mut graph = nested_global_face_cycle_geometry_graph();
        graph.global_face_id_by_global_dir_edge_id = vec![
            Some(0),
            Some(0),
        ...
        assert_eq!(
            ...
            PartitionBorderGlobalFaceRingClassificationStats {
                cycle_count: 2,
                classified_cycle_count: 2,
                shell_candidate_count: 1,
                unbounded_cycle_count: 1,
                containment_pair_count: 1,
                contained_cycle_count: 1,
                classification_ready: true,
                ..Default::default()
            }
        );
        assert_eq!(graph.global_face_ring_classifications().len(), 2);
        assert!(graph.global_face_ring_classifications()[0].shell_candidate);
        assert!(!graph.global_face_ring_classifications()[0].unbounded);
        assert_eq!(
            graph.global_face_ring_classifications()[1].containment_depth,
            1
        );
        assert!(graph.global_face_ring_classifications()[1].unbounded);
        let assembly_stats = graph
            .assemble_global_face_ring_candidates(&ExecutionPolicy::default(), stats)
            .unwrap();
        assert_eq!(assembly_stats.assembled_shell_count, 1);
        assert_eq!(assembly_stats.assigned_hole_count, 0);
        assert!(assembly_stats.candidate_ready);
        assert_eq!(graph.global_face_ring_candidate_assemblies().len(), 1);
        assert!(graph.global_face_ring_candidate_assemblies()[0]
            .hole_candidate_global_face_ids
            .is_empty());
    }

    #[test]
    fn global_face_ring_classification_rejects_ambiguous_interaction_atomically() {
        let mut graph = nested_global_face_cycle_geometry_graph();
        graph.global_face_id_by_global_dir_edge_id = vec![
            Some(0),
            Some(0),
        ...
        let payload_stats = graph
            ...
            .unwrap();
        let mut geometry_stats = PartitionBorderGlobalFaceCycleGeometryStats {
            edge_count: 8,
            cycle_count: 2,
            ring_payload_ready: true,
            endpoint_touch_count: 1,
            ..Default::default()
        };
        let stats = graph
            .classify_global_face_ring_payloads(
            ...
            .unwrap();
        assert_eq!(stats.ambiguous_interaction_count, 1);
        assert!(!stats.classification_ready);
        assert!(graph.global_face_ring_classifications().is_empty());

        geometry_stats.endpoint_touch_count = 0;
        let error = graph
            .classify_global_face_ring_payloads(
                &ExecutionPolicy {
        ...
        assert!(matches!(
            ...
            crate::PolygonizeError::ResourceLimitExceeded {
                ...
                limit: 0,
                observed: 4,
            } if stage == "partition_border_global_face_ring_classification_pairs"
        ));
    }

    #[test]
    fn global_face_ring_candidate_assembly_assigns_contained_holes() {
        let mut graph = cycle_geometry_graph(
            &[
                vec![(0.0, 0.0), (20.0, 0.0), (20.0, 20.0), (0.0, 20.0)],
                vec![(2.0, 2.0), (2.0, 8.0), (8.0, 8.0), (8.0, 2.0)],
        ...
        let classification_stats = graph
            ...
            .unwrap();
        assert!(classification_stats.classification_ready);
        let assembly_stats = graph
            .assemble_global_face_ring_candidates(&ExecutionPolicy::default(), classification_stats)
            .unwrap();
        assert_eq!(
            ...
            PartitionBorderGlobalFaceRingCandidateAssemblyStats {
                ...
                assembled_shell_count: 1,
                assigned_hole_count: 1,
                candidate_ready: true,
                ..Default::default()
            }
        );
        assert_eq!(
            graph.global_face_ring_candidate_assemblies()[0].hole_candidate_global_face_ids,
            vec![1]
        );
