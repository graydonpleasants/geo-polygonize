...
fn bench_simd_new(c: &mut Criterion) {
    ...
    let coords: Vec<Coord<f64>> = (0..1000).map(|_| Coord { x: rng.gen(), y: rng.gen() }).collect();

    c.bench_function("simd_ring_new", |b| {
        b.iter(|| SimdRing::new(&coords))
    });

    let coords3d: Vec<Coord3D> = (0..1000).map(|_| Coord3D { x: rng.gen(), y: rng.gen(), z: 0.0 }).collect();
