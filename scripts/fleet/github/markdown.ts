...
import { getIssues, EnhancedIssue } from "./issues.js";
import { getGitRepoInfo } from "./git.js";

function toIssueMarkdown(issue: EnhancedIssue): string {
  ...
  if (issue.linkedPrs && issue.linkedPrs.length > 0) {
    lines.push(`| **Linked PRs** | ${issue.linkedPrs.map(pr => `[#${pr.number} (branch: \`${pr.headRef}\`)](${pr.html_url})`).join(", ")} |`);
  }
...
async function toIssueDocMarkdown(issues: EnhancedIssue[]) {
